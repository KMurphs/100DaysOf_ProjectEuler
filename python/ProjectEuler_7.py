...
def test_for_primalityUsingPreviousPrimes(N, previous_primes):
	...
	for i in range(len(previous_primes)):
		current_prime = previous_primes[i]
...
def get_nth_prime_by_brute_force_using_previous_primes_and_2_increments(N):

	previous_primes = [2,3,5]
	previous_primes_count = 3 
	potential_prime = 7
	
	# prime above 5 are generated by 6k+1 or 6k+5 where k is a natural number greater than 0
	while (previous_primes_count < N): # 1 is not listed in previous primes and is not counted as prime number in the problem statement
		isPrime = True
		for i in range(previous_primes_count):
			current_prime = previous_primes[i]
		...
		if(isPrime):
			previous_primes.append(potential_prime)
			potential_prime += 2
			previous_primes_count += 1

	# print(f"{previous_primes}")
	return previous_primes[N-1]
...
if __name__ == "__main__": 
	...
	start = time.time()
	print(get_nth_prime_by_brute_force_using_previous_primes_and_2_increments(N))
	end = time.time()
	print(f"Operation took {1000*(end - start):.2f}ms")
